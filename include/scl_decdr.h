...
class SCL_DECODER{
    public: 
        ...
        std::vector<uint8_t> scl_deocder(std::vector<double> rx_signal, std::vector<int> B);
        //void __MEM_FREE__();
        void reset_data();
    ...
    private:
        ...
        void duplicatePath(uint32_t l, uint32_t phi, double z_bit, double o_bit,  std::pair<uint8_t, std::vector<uint8_t>> mkz,  std::pair<uint8_t, std::vector<uint8_t>> mko);
        ...
        void updatellrs_(double ***llr, uint8_t ***u_matrix, int l, int phi, uint16_t lst);
