...
SCL_DECODER* SCL_DECODER::Instance(uint32_t list_size,std::vector<uint8_t> reg){
    if(_instance==0){
...
void SCL_DECODER::duplicatePath(uint32_t l, uint32_t phi, double z_bit, double o_bit,std::pair<uint8_t, std::vector<uint8_t>> mkz,  std::pair<uint8_t, std::vector<uint8_t>> mko){
...
void SCL_DECODER::conitnueUnFrz_bit(uint32_t i){SCL_DECODER* scl= SCL_DECODER::Instance(L,g); 
    ...
    for(int l=0; l<L; ++l){
        if(activePath[l]){
            ...
            {
                probForks[2*l] = pathmetric(PM[l][i-1],llr[m][i][l],mkz.at(ci).first);
            
            
                probForks[2*l + 1] = pathmetric(PM[l][i-1],llr[m][i][l],mko.at(ci).first);                
            }
    ...
    std::sort(probabilities.begin(), probabilities.end(),std::greater<double>()); // in descending order 
    ...
    for(int l=0; l<L; ++l){
        ...
        if(contForks[2 * l ]==1 && contForks[2 * l + 1]==1){
            ...
            duplicatePath(l,i,probForks[2*l],probForks[2*l+1],mkz.at(pair_list_index.at(l)),mko.at(pair_list_index.at(l)));
...
std::vector<uint8_t> SCL_DECODER::scl_deocder(std::vector<double> rx_signal, std::vector<int> B){
    ///this->allocate_mem();
...
void SCL_DECODER::updatellrs_(double ***llr, uint8_t ***u_matrix, int l, int phi, uint16_t lst) {
    if (l == 0) {
    ...
    for (int beta = 0; beta < std::pow(2, std::log2(N) - l); ++beta) {
        ...
        if (phi % 2 == 0) {
            // first case here...
            llr[l][In_formation_(phi, l, beta)][lst] = (double)SIGN(a) * (double)SIGN(b) * (double) MIN(ABS(a), ABS(b));
        }
        else {
            // second case here
            int u_p = (int)u_matrix[l][In_formation_(phi - 1, l, beta)][lst];
            llr[l][In_formation_(phi, l, beta)][lst] = ((1 - 2 * (double)u_p)) * a + b;
        }
    }


}
void SCL_DECODER::updatepartialsum_(uint8_t ***u_matrix, int l, int phi, uint16_t lst) {
    int si = phi / 2;
    int l_ = l - 1;
    ...
    for (int beta = 0; beta < std::pow(2, m - l); ++beta) {
        ...
        u_matrix[l_][In_formation_(si, l_, 2 * beta)][lst] = u_matrix[l][In_formation_(phi - 1, l, beta)][lst] ^ u_matrix[l][In_formation_(phi, l, beta)][lst];
        u_matrix[l_][In_formation_(si, l_, 2 * beta + 1)][lst] = u_matrix[l][In_formation_(phi, l, beta)][lst];
    }
