...
std::pair<int, std::vector<uint8_t>> pac_decoding(std::vector<double> rx_signal,
  ...
  while (i != N) {
    ...
    if (frozen_check(i, Bvec)) {
      ...
      double mu_0 = cal_PM(i - 1, llr, u_matrix) +
                    actual_metric_cal(cur_llr, curpr0.first) - exp_metric;
      double mu_1 = cal_PM(i - 1, llr, u_matrix) +
                    actual_metric_cal(cur_llr, curpr1.first) - exp_metric;
  ...
  std::pair<int, std::vector<uint8_t>> r_pair =
      std::make_pair(v_nodes, msg_cap);
  return r_pair;
}
...
void updatepartialsum(std::vector<std::vector<uint8_t>> &u_matrix, int l,
  ...
  for (int beta = 0; beta < std::pow(2, std::log2(N) - l); ++beta) {
    ...
    u_matrix[l_][In_formation(si, l_, 2 * beta)] =
        u_matrix[l][In_formation(phi - 1, l, beta)] ^
        u_matrix[l][In_formation(phi, l, beta)];
    u_matrix[l_][In_formation(si, l_, 2 * beta + 1)] =
        u_matrix[l][In_formation(phi, l, beta)];
  }
