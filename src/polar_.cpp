...
int main()
{
    ...
    std::cout << std::endl; 
    // if(!pac){
    //     B_unsorted = {43,  84,  98,  71,  88,  45, 100,  51,  46, 104,  75,  53, 112,
    //     54,  77,  57,  83,  78,  58,  31,  85,  60,  99,  86,  89, 101,
	...
	// B_unsorted = { 97,  84,  98,  88,  45,  71, 100,  46,  51, 104, 112,  75,  53,
    ...
    // std::cout << std::endl;
            ...
            if (blk_errs >= 110) {
                std::cout << "BLK: " << blk + 1 << ",SNR: " << p_snr << ",FER: " << blk_errs << ",BER: " << bit_errs << "\n";
                std::ofstream outfile;
                outfile.open("sim_result.txt", std::fstream::app);

                outfile << "SCL: L: " <<L <<",BLK:," << blk + 1 << ",SNR:," << p_snr << "(dB),FER:," << blk_errs << ",BER:," << bit_errs << ",N:," << N << ",K:," << K << ",Rate: " << Rate << "\n";
		...
		if((blk+1)%20==0){
           std::cout << "SCL: L: " << L << ", BLK: " << blk + 1 << ",SNR: " << p_snr << "(dB),FER: " << blk_errs << ",BER: " << bit_errs << ",N: " << N << ",K: " << K << ",Rate: " << Rate << "\n";
		}
